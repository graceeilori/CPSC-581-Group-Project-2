...
io.on("connection", (socket) => {
  ...
  socket.on("session:create", ({ expertName }, callback) => {
    ...
    console.log(`Session created: ${code} by ${expertName}`);
    callback({ code }); // send code back to expert
  });
  ...
  socket.on("session:join", ({ code, studentName }, callback) => {
    ...
    callback({ success: true, expertName: session.expertName });
